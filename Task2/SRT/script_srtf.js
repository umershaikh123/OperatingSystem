...
//   for (let i = 0; i < numProcesses; i++) {
//     let segments = []
//     let lastEndTime = processes[i].startTime
//     let lastColor

//     while (lastEndTime < processes[i].completionTime) {
//       let nextEndTime = processes[i].completionTime

//       // Find the next segment end time (i.e., time until interrupted by another process)
//       for (let j = i + 1; j < numProcesses; j++) {
//         if (processes[j].startTime > lastEndTime) {
//           nextEndTime = Math.min(nextEndTime, processes[j].startTime)
//           break
//         }
//       }
...
//         endTime: nextEndTime,
//         color: lastColor || getRandomColor(),
//       })

//       lastEndTime = nextEndTime
//       lastColor = segments[segments.length - 1].color
//     }
...
//   // Adjusted visualization data for proper display without overlapping colors
//   const adjustedVisualizationData = visualizationData.map(processData => {
//     const mergedSegments = []
//     let currentSegment = null

//     processData.segments.forEach(segment => {
//       if (!currentSegment || currentSegment.color !== segment.color) {
//         if (currentSegment) {
...
//   // Draw rectangles for each process with merged segments
//   svg
//     .selectAll("g")
//     .data(adjustedVisualizationData)
//     .enter()
//     .append("g")
//     .selectAll("rect")
//     .data(d => d.segments)
...
//     .append("text")
//     .attr("x", d => xScale(d.startTime) + 5)
//     .attr("y", d => 40 + d.process * 40)
//     .attr("dy", "1em") // Adjust the vertical position of the text
//     .text(d => `P${d3.select(this.parentNode).datum().process}`)
//     .attr("font-size", "14px")
//     .attr("fill", "black")
